...
First, check if `/docs/hlds/{feature-name}.md` already exists:
...
```
HLD Sections Status for: {feature-name}
─────────────────────────────────────
[ ] 1.  Problem Statement
[ ] 2.  Goals
[ ] 3.  Non-Goals
...
Ask user to confirm: "Ready to write the HLD to `/docs/hlds/{feature-name}.md`?"
